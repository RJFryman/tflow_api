...
describe Tflow::ApiClient::Order do
  before(:each) do
   ...
   it 'it should create an order' do
     stub_request(:post, 'http://10.0.0.1/api/v2/order/create').to_return(:headers => {"Content-Type"=> "application/json"}, :body => fixture("order.json"))
     json_payload = {
     	"id": 1,
   ...
   it 'should list all orders' do
     stub_request(:get, 'http://10.0.0.1/api/v2/order/list').to_return(:headers => {"Content-Type"=> "application/json"}, :body => fixture("orders.json"))
   ...
   it 'should get a specific order based on id' do
     stub_request(:get, 'http://10.0.0.1/api/v2/order/100').to_return(:headers => {"Content-Type"=> "application/json"}, :body => fixture("order.json"))
   ...
   it 'should delete an order' do
     ...
     response = @client.delete_order('100')
     response.should == "nothing"
   end

   it 'should update an order' do
     stub_request(:post, 'http://10.0.0.1/api/v2/order/1/update').to_return(:headers => {"Content-Type"=> "application/json"}, :body => fixture("order.json"))
     json_payload = {
     	"id": 1,
     	"name": "Name for very first order",
     	"description": "Description for the order",
     ...
     response = @client.update_order(1, json_payload)
     response.should == {"id"=>1, "name"=>"Name for very first order", "description"=>"Description for the order", "ship_date"=>"2016-01-01T00:00:00+0100", "kfpx"=>"Passthrough", "assignments"=>{"upload_revision"=>[{"user_id"=>100}], "approve_jobs"=>[{"user_id"=>100}, {"user_id"=>101}], "release_jobs"=>[{"user_id"=>210}, {"user_id"=>100}], "upload_proofs"=>[{"user_id"=>132}]}, "tflows"=>[{"id"=>"808:1"}], "props"=>{"print_width"=>"47.56", "print_height"=>"50.24", "min_ppi"=>"330", "product_types"=>nil, "rep_name"=>nil, "pm_name"=>nil}}
   end
